...
from tensorflow.keras.layers import Input, Lambda, Conv2D, Reshape, TimeDistributed
from tensorflow.keras.models import Model

from notekeras.model.faster_rcnn.layers.anchors import Anchor
from notekeras.model.faster_rcnn.layers.base_net import resnet50
from notekeras.model.faster_rcnn.layers.clip_boxes import ClipBoxes
from notekeras.model.faster_rcnn.layers.detect_boxes import ProposalToDetectBox
from notekeras.model.faster_rcnn.layers.losses import rpn_cls_loss, rpn_regress_loss, detect_regress_loss, \
    detect_cls_loss
from notekeras.model.faster_rcnn.layers.proposals import RpnToProposal
from notekeras.model.faster_rcnn.layers.roi_align import RoiAlign
from notekeras.model.faster_rcnn.layers.specific_to_agnostic import deal_delta
from notekeras.model.faster_rcnn.layers.target import RpnTarget, DetectTarget
from notekeras.model.faster_rcnn.utils.utils import log
...
def rpn(base_layers, num_anchors, l2_reg=5e-4):
    x = Conv2D(512, (3, 3), padding='same', use_bias=False, kernel_initializer='he_normal',
               kernel_regularizer=regularizers.l2(l2_reg), name='rpn_conv')(base_layers)
    ...
    x_class = Conv2D(num_anchors * 2, (1, 1), use_bias=False, kernel_regularizer=regularizers.l2(l2_reg),
                     activation='linear', name='rpn_class_logits')(x)
    x_class = Reshape((-1, 2))(x_class)
    x_regr = Conv2D(num_anchors * 4, (1, 1), use_bias=False, kernel_regularizer=regularizers.l2(l2_reg),
                    activation='linear', name='rpn_deltas')(x)
...
def rcnn(base_layers, rois, num_classes, image_max_dim, head_fn, pool_size=(7, 7), fc_layers_size=1024, l2_reg=5e-4):
    ...
    # 分类
    class_logits = TimeDistributed(layers.Dense(num_classes, use_bias=False, kernel_regularizer=regularizers.l2(l2_reg),
                                                activation='linear'), name='rcnn_class_logits')(shared_layer)

    # 回归(类别相关)
    deltas = TimeDistributed(layers.Dense(4 * num_classes, use_bias=False, kernel_regularizer=regularizers.l2(l2_reg),
                                          activation='linear'), name='rcnn_deltas')(
