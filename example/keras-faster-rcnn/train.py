...
from notekeras.model.faster_rcnn.config import current_config as config
from notekeras.model.faster_rcnn.layers import models
from notekeras.model.faster_rcnn.preprocess.input import VocDataset
from notekeras.model.faster_rcnn.utils import model_utils
from notekeras.model.faster_rcnn.utils.generator import Generator
...
def main(args):
    ...
    # 生成器
    val_gen = Generator(test_img_info[:500], config.IMAGE_INPUT_SHAPE, config.MEAN_PIXEL, config.BATCH_SIZE,
                        config.MAX_GT_INSTANCES)
    # 训练conv3 及以上
